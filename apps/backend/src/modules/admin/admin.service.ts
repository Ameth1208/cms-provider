  ...
  async createClient(data: {
    ...
    const org = await this.prisma.organization.create({
      data: {
        ...
        // No plan system - all clients have same access level
      },
    })
    ...
    // Assign catalog and orders permissions to MANAGER
    const managerPermissions = allPermissions.filter(
      (p) => p.resource === 'catalog' || p.resource === 'orders' || p.resource === 'inventory' || p.resource === 'campaigns',
    )
    await this.prisma.rolePermission.createMany({
